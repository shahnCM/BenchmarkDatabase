...
func FilterQuery(conn *pgx.Conn) error {
	...
	ipAddress := os.Getenv("FILTER_QUERY_QUESTDB_IP_ADDRESS")
	if ipAddress == "" {
		log.Fatal("FILTER_QUERY_QUESTDB_IP_ADDRESS is not set in .env")
	}
	...
	// Execute the query
	rows, err := conn.Query(context.Background(), query, ipAddress)
	if err != nil {
	...
	for rows.Next() {
		...
		fmt.Printf("user_id: %s, cpu_id: %s, device_id: %s, app_id: %s, ip_address: %s, mac_address: %s, usage_by_user: %.2f, usage_by_idle: %.2f, usage_by_system: %.2f, dump: %s\n",
